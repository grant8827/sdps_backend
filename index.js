...
import { db, id, passwordHash } from './database.js';
...
app.post('/api/auth/login', (req, res) => {
  const session = login(String(req.body.identifier || '').trim(), String(req.body.password || ''));
...
app.get('/api/me/schools', requireAuth, (req, res) => {
  res.json(getMemberships(req.user.id));
});
...
app.post('/api/me/change-password', requireAuth, (req, res) => {
  ...
  db.prepare('UPDATE users SET password_hash=? WHERE id=?').run(passwordHash(newPassword), req.user.id);
  res.status(204).end();
});
...
app.post('/api/auth/register-school', (req, res) => {
  ...
  try {
    ...
    for (let attempt = 0; db.prepare('SELECT 1 FROM schools WHERE code=?').get(code); attempt++) {
    ...
    db.prepare('INSERT INTO organizations (id,name) VALUES (?,?)').run(organizationId, schoolName.trim());

    const schoolId = id('school');
    db.prepare('INSERT INTO schools (id,organization_id,name,code) VALUES (?,?,?,?)').run(schoolId, organizationId, schoolName.trim(), code);

    const campusId = id('campus');
    db.prepare('INSERT INTO campuses (id,school_id,name,address) VALUES (?,?,?,?)').run(campusId, schoolId, campusName.trim(), campusAddress?.trim() || null);

    const userId = id('admin');
    db.prepare(`INSERT INTO users (id,full_name,email,password_hash,role) VALUES (?,?,?,?,'admin')`).run(userId, adminFullName.trim(), email.trim(), passwordHash(password));
    db.prepare(`INSERT INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,NULL,'school_admin')`).run(id('membership'), userId, schoolId);
    ...
    db.prepare(`INSERT INTO school_years (id,school_id,name,starts_on,ends_on,status) VALUES (?,?,?,?,?,'ACTIVE')`)
      .run(id('year'), schoolId, `${startYear}-${startYear + 1}`, `${startYear}-08-01`, `${startYear + 1}-06-30`);

    db.exec('COMMIT');
    res.status(201).json(login(email.trim(), password));
  } catch (error) {
    db.exec('ROLLBACK');
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'That email is already in use.' : error.message });
  }
});
...
app.get('/api/me/students', requireAuth, requireRole('parent'), (req, res) => {
  const rows = db.prepare(`${studentSelect} JOIN student_guardians sg ON sg.student_id=s.id JOIN guardians gu ON gu.id=sg.guardian_id JOIN memberships m ON m.user_id=gu.user_id AND m.school_id=s.school_id AND m.role='parent' AND m.status='ACTIVE' WHERE gu.user_id=? AND y.status='ACTIVE' AND s.status='ACTIVE' GROUP BY s.id,e.id ORDER BY sc.name,s.last_name,s.first_name`).all(req.user.id);
...
app.get('/api/me/attendance', requireAuth, requireRole('parent'), (req, res) => {
...
function createQueueRequest(req, res, requestType, requiredStatus, nextStatus) {
  const link = guardianLinkQuery.get(req.params.studentId, req.user.id);
  ...
  const context = studentContextQuery.get(req.params.studentId);
  ...
  try {
    ...
    db.prepare(`INSERT INTO queue_items (id,school_id,campus_id,student_id,teacher_user_id,request_type,requested_by_user_id) VALUES (?,?,?,?,?,?,?)`)
      .run(itemId, context.schoolId, context.campusId, req.params.studentId, context.teacherId, requestType, req.user.id);
    db.prepare('UPDATE students SET pickup_status=? WHERE id=?').run(nextStatus, req.params.studentId);
    db.exec('COMMIT');
...
app.post('/api/me/students/:studentId/drop-off', requireAuth, requireRole('parent'), (req, res) => {
  createQueueRequest(req, res, 'DROP_OFF', 'AT_HOME', 'DROPOFF_REQUESTED');
});

app.post('/api/me/students/:studentId/pick-up', requireAuth, requireRole('parent'), (req, res) => {
  createQueueRequest(req, res, 'PICK_UP', 'PRESENT', 'PICKUP_REQUESTED');
});
...
app.get('/api/teacher/queue', requireAuth, requireRole('teacher'), (req, res) => {
  res.json(db.prepare(`${queueSelect} WHERE qi.status='PENDING' AND qi.teacher_user_id=? ORDER BY qi.requested_at ASC`).all(req.user.id));
});

app.get('/api/admin/queue', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  res.json(db.prepare(`${queueSelect} WHERE qi.status='PENDING' AND qi.school_id=? ORDER BY qi.requested_at ASC`).all(req.school.id));
});
...
app.post('/api/queue/:id/approve', requireAuth, (req, res) => {
  const item = db.prepare(`SELECT * FROM queue_items WHERE id=? AND status='PENDING'`).get(req.params.id);
  ...
  const isSchoolAdmin = getMemberships(req.user.id).some(m => m.schoolId === item.school_id && ['school_admin', 'platform_super_admin'].includes(m.role));
  if (!isOwningTeacher && !isSchoolAdmin) return res.status(403).json({ error: 'You do not have permission to approve this request.' });
  try {
    db.exec('BEGIN IMMEDIATE');
    db.prepare(`UPDATE queue_items SET status='APPROVED', approved_at=CURRENT_TIMESTAMP, approved_by_user_id=? WHERE id=?`).run(req.user.id, item.id);
    db.prepare('UPDATE students SET pickup_status=? WHERE id=?').run(item.request_type === 'DROP_OFF' ? 'PRESENT' : 'PICKED_UP', item.student_id);
    ...
    if (item.request_type === 'DROP_OFF') {
      const context = studentContextQuery.get(item.student_id);
      const school = db.prepare('SELECT timezone, start_time AS startTime FROM schools WHERE id=?').get(item.school_id);
      const campus = item.campus_id ? db.prepare('SELECT timezone, start_time AS startTime FROM campuses WHERE id=?').get(item.campus_id) : null;
      ...
      db.prepare(`
        ...
        ON CONFLICT(student_id,date) DO UPDATE SET status=excluded.status, marked_by_user_id=excluded.marked_by_user_id, marked_at=CURRENT_TIMESTAMP, late=excluded.late`)
...
app.post('/api/queue/:id/decline', requireAuth, (req, res) => {
  const item = db.prepare(`SELECT * FROM queue_items WHERE id=? AND status='PENDING'`).get(req.params.id);
  if (!item) return res.status(404).json({ error: 'Request not found or already handled.' });
  const isOwningTeacher = req.user.role === 'teacher' && item.teacher_user_id === req.user.id;
  const isSchoolAdmin = getMemberships(req.user.id).some(m => m.schoolId === item.school_id && ['school_admin', 'platform_super_admin'].includes(m.role));
  ...
  try {
    db.exec('BEGIN IMMEDIATE');
    db.prepare(`UPDATE queue_items SET status='DECLINED', declined_at=CURRENT_TIMESTAMP, declined_by_user_id=? WHERE id=?`).run(req.user.id, item.id);
    db.prepare('UPDATE students SET pickup_status=? WHERE id=?').run(item.request_type === 'DROP_OFF' ? 'AT_HOME' : 'PRESENT', item.student_id);
    db.exec('COMMIT');
    res.status(204).end();
  } catch (error) {
    db.exec('ROLLBACK');
...
// A day with no explicit record defaults to WEEKEND on Sat/Sun (SQLite
// strftime('%w', date) is 0=Sun..6=Sat) and UNMARKED otherwise — the
// teacher can still override either default by marking that date
// explicitly, same as any other status.
const attendanceSelect = `
  SELECT s.id AS studentId, s.first_name || ' ' || s.last_name AS fullName, s.photo_url AS photoUrl,
    ...
    IFNULL(ar.status, CASE WHEN strftime('%w', ?) IN ('0','6') THEN 'WEEKEND' ELSE 'UNMARKED' END) AS status
  FROM students s
  JOIN student_enrollments e ON e.student_id=s.id
  JOIN school_years y ON y.id=e.school_year_id AND y.status='ACTIVE'
...
app.get('/api/teacher/attendance', requireAuth, requireRole('teacher'), (req, res) => {
  const date = req.query.date || todayIso();
  res.json(db.prepare(`${attendanceSelect} AND c.teacher_user_id=? ORDER BY s.last_name,s.first_name`).all(date, date, req.user.id));
});
...
app.get('/api/teacher/class', requireAuth, requireRole('teacher'), (req, res) => {
  const myClass = db.prepare(`
...
app.get('/api/admin/attendance', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  if (!req.query.classId) return res.status(400).json({ error: 'classId is required' });
  const date = req.query.date || todayIso();
  res.json(db.prepare(`${attendanceSelect} AND s.school_id=? AND e.class_id=? ORDER BY s.last_name,s.first_name`).all(date, date, req.school.id, req.query.classId));
});
...
app.post('/api/attendance', requireAuth, (req, res) => {
  ...
  const isSchoolAdmin = getMemberships(req.user.id).some(m => m.schoolId === context.schoolId && ['school_admin', 'platform_super_admin'].includes(m.role));
  ...
  db.prepare(`
    ...
    ON CONFLICT(student_id,date) DO UPDATE SET status=excluded.status, marked_by_user_id=excluded.marked_by_user_id, marked_at=CURRENT_TIMESTAMP`)
    .run(id('attendance'), context.schoolId, context.campusId, studentId, context.classId, date, status, req.user.id);
  res.status(204).end();
});

app.get('/api/admin/overview', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const totalStudents = db.prepare(`SELECT COUNT(*) AS c FROM students WHERE school_id=? AND status='ACTIVE'`).get(req.school.id).c;
  const activeTeachers = db.prepare(`SELECT COUNT(*) AS c FROM memberships WHERE school_id=? AND role='teacher' AND status='ACTIVE'`).get(req.school.id).c;
  const presentToday = db.prepare(`SELECT COUNT(*) AS c FROM students WHERE school_id=? AND status='ACTIVE' AND pickup_status='PRESENT'`).get(req.school.id).c;
  const pendingRequests = db.prepare(`SELECT COUNT(*) AS c FROM queue_items WHERE school_id=? AND status='PENDING'`).get(req.school.id).c;
  res.json({ totalStudents, activeTeachers, presentToday, pendingRequests });
});

app.get('/api/admin/setup', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  res.json({
    school: db.prepare('SELECT id,name,code,timezone,status,start_time AS startTime,dismissal_time AS dismissalTime,extended_time AS extendedTime FROM schools WHERE id=?').get(req.school.id),
    campuses: db.prepare('SELECT id,name,address,latitude,longitude,geofence_radius AS geofenceRadius,timezone,status,start_time AS startTime,dismissal_time AS dismissalTime,extended_time AS extendedTime FROM campuses WHERE school_id=? ORDER BY name').all(req.school.id),
    schoolYears: db.prepare('SELECT * FROM school_years WHERE school_id=? ORDER BY starts_on DESC').all(req.school.id),
    gradeLevels: db.prepare('SELECT id,name,sort_order AS sortOrder,next_grade_level_id AS nextGradeLevelId FROM grade_levels ORDER BY sort_order').all(),
    classes: db.prepare('SELECT id,name,room_name AS roomName,school_year_id AS schoolYearId,grade_level_id AS gradeLevelId,teacher_user_id AS teacherId,campus_id AS campusId FROM classes WHERE school_id=? ORDER BY name').all(req.school.id),
    guardians: db.prepare(`SELECT DISTINCT gu.id,u.full_name AS fullName,u.email,u.phone FROM guardians gu JOIN users u ON u.id=gu.user_id JOIN memberships m ON m.user_id=u.id WHERE m.school_id=? AND m.status='ACTIVE' ORDER BY u.full_name`).all(req.school.id),
  });
});
...
app.patch('/api/admin/school', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  const current = db.prepare('SELECT name,start_time AS startTime,dismissal_time AS dismissalTime,extended_time AS extendedTime FROM schools WHERE id=?').get(req.school.id);
  ...
  db.prepare('UPDATE schools SET name=?, start_time=?, dismissal_time=?, extended_time=? WHERE id=?')
...
app.post('/api/admin/campuses', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  const campusId = id('campus');
  try {
    db.prepare('INSERT INTO campuses (id,school_id,name,address,start_time,dismissal_time,extended_time) VALUES (?,?,?,?,?,?,?)')
    ...
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'A location with that name already exists' : error.message });
  }
});

app.patch('/api/admin/campuses/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const { name, address, startTime, dismissalTime, extendedTime } = req.body;
  for (const [label, value] of [['startTime', startTime], ['dismissalTime', dismissalTime], ['extendedTime', extendedTime]]) {
    if (value !== undefined && value !== null && value !== '' && !isValidClockTime(value)) {
      return res.status(400).json({ error: `${label} must be a HH:MM time` });
  ...
  const current = db.prepare('SELECT name,address,start_time AS startTime,dismissal_time AS dismissalTime,extended_time AS extendedTime FROM campuses WHERE id=? AND school_id=?').get(req.params.id, req.school.id);
  ...
  try {
    db.prepare('UPDATE campuses SET name=?, address=?, start_time=?, dismissal_time=?, extended_time=? WHERE id=?')
...
app.get('/api/admin/classes', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  res.json(db.prepare(`
...
app.post('/api/admin/classes', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  const year = db.prepare('SELECT id FROM school_years WHERE id=? AND school_id=?').get(schoolYearId, req.school.id);
  ...
  if (!resolvedCampusId) {
    resolvedCampusId = db.prepare('SELECT id FROM campuses WHERE school_id=? ORDER BY name LIMIT 1').get(req.school.id)?.id || null;
  }
  try {
    const classId = id('class');
    db.prepare(`INSERT INTO classes (id,name,room_name,school_year_id,grade_level_id,campus_id,school_id) VALUES (?,?,?,?,?,?,?)`)
    ...
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'A class with this name already exists for that school year' : error.message });
  }
});

app.get('/api/admin/teachers', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const rows = db.prepare(`
...
app.post('/api/admin/teachers', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  if (classId) {
    assignedClass = db.prepare('SELECT id, campus_id AS campusId FROM classes WHERE id=? AND school_id=?').get(classId, req.school.id);
  ...
  try {
    ...
    db.prepare(`INSERT INTO users (id,full_name,email,password_hash,photo_url,role) VALUES (?,?,?,?,?,'teacher')`)
      .run(userId, fullName.trim(), email.trim(), passwordHash(password), photoUrl);
    db.prepare(`INSERT INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,?, 'teacher')`)
      .run(id('membership'), userId, req.school.id, assignedClass?.campusId || null);
    if (assignedClass) db.prepare('UPDATE classes SET teacher_user_id=? WHERE id=?').run(userId, assignedClass.id);
    db.exec('COMMIT');
...
app.patch('/api/admin/teachers/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const membership = db.prepare(`SELECT 1 FROM memberships WHERE user_id=? AND school_id=? AND role='teacher'`).get(req.params.id, req.school.id);
  ...
  if (fullName?.trim()) db.prepare('UPDATE users SET full_name=? WHERE id=?').run(fullName.trim(), req.params.id);
  if (photoUrl !== undefined) db.prepare('UPDATE users SET photo_url=? WHERE id=?').run(photoUrl, req.params.id);
  if (classId !== undefined) {
    db.prepare(`UPDATE classes SET teacher_user_id=NULL WHERE teacher_user_id=? AND school_id=?`).run(req.params.id, req.school.id);
    if (classId) {
      const cls = db.prepare('SELECT id FROM classes WHERE id=? AND school_id=?').get(classId, req.school.id);
      if (!cls) return res.status(400).json({ error: 'That classroom does not belong to this school' });
      db.prepare('UPDATE classes SET teacher_user_id=? WHERE id=?').run(req.params.id, classId);
    }
  }
  res.status(204).end();
});

app.get('/api/admin/students', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const active = db.prepare(`${studentSelect} WHERE s.school_id=? AND y.status='ACTIVE' ORDER BY s.last_name,s.first_name`).all(req.school.id);
  ...
  res.json(active.map(student => ({ ...student, status: student.pickupStatus, enrollmentStatus: student.status, daycare: Boolean(student.daycare), guardians: guardianQuery.all(student.id) })));
});

app.post('/api/admin/students', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  const selectedClass = db.prepare('SELECT campus_id FROM classes WHERE id=? AND school_id=? AND school_year_id=? AND grade_level_id=?').get(classId, req.school.id, schoolYearId, gradeLevelId);
  ...
  try {
    ...
    if (!guardianId && guardian?.email) {
      const existing = db.prepare(`SELECT gu.id,u.id AS user_id FROM guardians gu JOIN users u ON u.id=gu.user_id WHERE u.email=? COLLATE NOCASE`).get(guardian.email.trim());
      if (existing) {
        guardianId = existing.id;
        db.prepare(`INSERT OR IGNORE INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,?, 'parent')`).run(id('membership'), existing.user_id, req.school.id, null);
      }
      else {
        ...
        db.prepare(`INSERT INTO users (id,full_name,email,phone,password_hash,role) VALUES (?,?,?,?,?,'parent')`).run(userId, guardian.fullName.trim(), guardian.email.trim(), guardian.phone || null, passwordHash(guardian.temporaryPassword));
        db.prepare('INSERT INTO guardians (id,user_id) VALUES (?,?)').run(guardianId, userId);
        db.prepare(`INSERT INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,?, 'parent')`).run(id('membership'), userId, req.school.id, null);
      }
    }
    if (guardianId) {
      const permittedGuardian = db.prepare(`SELECT 1 FROM guardians gu JOIN memberships m ON m.user_id=gu.user_id WHERE gu.id=? AND m.school_id=? AND m.status='ACTIVE'`).get(guardianId, req.school.id);
      if (!permittedGuardian) throw new Error('The selected parent does not belong to this school');
    }
    const studentId = id('student');
    db.prepare(`INSERT INTO students (id,first_name,last_name,date_of_birth,student_number,photo_url,daycare,school_id,campus_id) VALUES (?,?,?,?,?,?,?,?,?)`).run(studentId, firstName.trim(), lastName.trim(), dateOfBirth || null, studentNumber?.trim() || null, photoUrl, daycare ? 1 : 0, req.school.id, selectedClass.campus_id);
    db.prepare(`INSERT INTO student_enrollments (id,student_id,school_year_id,grade_level_id,class_id,school_id,campus_id) VALUES (?,?,?,?,?,?,?)`).run(id('enrollment'), studentId, schoolYearId, gradeLevelId, classId, req.school.id, selectedClass.campus_id);
    if (guardianId) db.prepare(`INSERT INTO student_guardians (student_id,guardian_id,relationship,is_primary,can_pick_up,can_manage) VALUES (?,?,?,1,?,1)`).run(studentId, guardianId, guardian.relationship || 'Guardian', guardian.canPickUp === false ? 0 : 1);
    db.exec('COMMIT');
    res.status(201).json({ id: studentId });
  } catch (error) {
    db.exec('ROLLBACK');
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'Student number or guardian email already exists' : error.message });
  }
});

app.post('/api/admin/students/:studentId/guardians', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const { guardianId, relationship = 'Guardian', canPickUp = true, canManage = true } = req.body;
  try {
    const allowed = db.prepare(`SELECT 1 FROM students s JOIN guardians gu ON gu.id=? JOIN memberships m ON m.user_id=gu.user_id AND m.school_id=s.school_id AND m.status='ACTIVE' WHERE s.id=? AND s.school_id=?`).get(guardianId, req.params.studentId, req.school.id);
    if (!allowed) return res.status(404).json({ error: 'Student or guardian not found in this school' });
    db.prepare(`INSERT INTO student_guardians (student_id,guardian_id,relationship,can_pick_up,can_manage) VALUES (?,?,?,?,?) ON CONFLICT(student_id,guardian_id) DO UPDATE SET relationship=excluded.relationship,can_pick_up=excluded.can_pick_up,can_manage=excluded.can_manage`).run(req.params.studentId, guardianId, relationship, canPickUp ? 1 : 0, canManage ? 1 : 0);
    res.status(204).end();
...
app.patch('/api/admin/students/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const status = req.body.status === 'SUSPENDED' ? 'SUSPENDED' : 'ACTIVE';
  const result = db.prepare('UPDATE students SET status=? WHERE id=? AND school_id=?').run(status, req.params.id, req.school.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Student not found' });
  res.status(204).end();
});

app.delete('/api/admin/students/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const result = db.prepare(`UPDATE students SET status='ARCHIVED' WHERE id=? AND school_id=?`).run(req.params.id, req.school.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Student not found' });
  res.status(204).end();
});

app.get('/api/admin/guardians', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const guardians = db.prepare(`SELECT DISTINCT gu.id, u.full_name AS fullName, u.email, u.phone, m.status FROM guardians gu JOIN users u ON u.id=gu.user_id JOIN memberships m ON m.user_id=u.id WHERE m.school_id=? AND m.role='parent' ORDER BY u.full_name`).all(req.school.id);
  ...
  res.json(guardians.map(g => ({ ...g, active: g.status === 'ACTIVE', children: children.all(g.id, req.school.id) })));
});

app.post('/api/admin/guardians', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  try {
    db.exec('BEGIN IMMEDIATE');
    let user = db.prepare('SELECT id FROM users WHERE email=? COLLATE NOCASE').get(email.trim());
    let guardianId;
    if (user) {
      ...
      db.prepare(`INSERT INTO users (id,full_name,email,phone,password_hash,role) VALUES (?,?,?,?,?,'parent')`).run(user.id, fullName.trim(), email.trim(), phone?.trim() || null, passwordHash(temporaryPassword));
      db.prepare('INSERT INTO guardians (id,user_id) VALUES (?,?)').run(guardianId, user.id);
    }
    db.prepare(`INSERT INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,?, 'parent')`).run(id('membership'), user.id, req.school.id, null);
    db.exec('COMMIT');
    res.status(201).json({ id: guardianId });
  } catch (error) {
    try { db.exec('ROLLBACK'); } catch {}
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'Parent already belongs to this school' : error.message });
  }
});

app.patch('/api/admin/guardians/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const guardian = db.prepare('SELECT user_id FROM guardians WHERE id=?').get(req.params.id);
  if (!guardian) return res.status(404).json({ error: 'Guardian not found' });
  const result = db.prepare(`UPDATE memberships SET status=? WHERE user_id=? AND school_id=? AND role='parent'`).run(req.body.active ? 'ACTIVE' : 'SUSPENDED', guardian.user_id, req.school.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Parent membership not found in this school' });
  res.status(204).end();
});

app.delete('/api/admin/guardians/:id', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const guardian = db.prepare('SELECT user_id FROM guardians WHERE id=?').get(req.params.id);
  if (!guardian) return res.status(404).json({ error: 'Guardian not found' });
  const result = db.prepare(`UPDATE memberships SET status='ARCHIVED' WHERE user_id=? AND school_id=? AND role='parent'`).run(guardian.user_id, req.school.id);
  db.prepare(`DELETE FROM student_guardians WHERE guardian_id=? AND student_id IN (SELECT id FROM students WHERE school_id=?)`).run(req.params.id, req.school.id);
  if (result.changes === 0) return res.status(404).json({ error: 'Parent membership not found in this school' });
  res.status(204).end();
});

app.get('/api/admin/promotions/preview', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const from = req.query.from; const to = req.query.to;
  const rows = db.prepare(`${studentSelect} WHERE e.school_year_id=? AND s.school_id=? AND e.status='ENROLLED' ORDER BY s.last_name,s.first_name`).all(from, req.school.id);
  ...
  res.json(rows.map(row => ({ studentId: row.id, fullName: row.fullName, fromGrade: row.gradeName, proposedGrade: next.get(row.gradeLevelId) || null, alreadyEnrolled: Boolean(exists.get(row.id, to)) })));
});

app.post('/api/admin/promotions', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  const { fromSchoolYearId, toSchoolYearId, overrides = {} } = req.body;
  try {
    db.exec('BEGIN IMMEDIATE');
    const validYears = db.prepare(`SELECT count(*) AS count FROM school_years WHERE id IN (?,?) AND school_id=?`).get(fromSchoolYearId, toSchoolYearId, req.school.id);
    if (validYears.count !== 2) throw new Error('School years do not belong to this school');
    db.prepare('INSERT INTO promotion_runs (id,from_school_year_id,to_school_year_id,created_by,school_id) VALUES (?,?,?,?,?)').run(id('promotion'), fromSchoolYearId, toSchoolYearId, req.user.id, req.school.id);
    const enrollments = db.prepare(`SELECT e.*,g.next_grade_level_id FROM student_enrollments e JOIN grade_levels g ON g.id=e.grade_level_id WHERE e.school_year_id=? AND e.school_id=? AND e.status='ENROLLED'`).all(fromSchoolYearId, req.school.id);
    ...
    for (const enrollment of enrollments) {
      ...
      if (!gradeId) continue;
      insert.run(id('enrollment'), enrollment.student_id, toSchoolYearId, gradeId, choice.status || 'ENROLLED', enrollment.id, req.school.id, enrollment.campus_id); promoted++;
    }
    db.exec('COMMIT'); res.status(201).json({ promoted });
  } catch (error) { db.exec('ROLLBACK'); res.status(409).json({ error: 'This promotion has already run, or next-year enrollments already exist.' }); }
});
...
app.post('/api/admin/promotions/by-grade', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  let classId = null;
  if (teacherUserId) {
    const teacherClass = db.prepare(`SELECT id FROM classes WHERE teacher_user_id=? AND school_id=?`).get(teacherUserId, req.school.id);
  ...
  try {
    ...
    for (const enrollment of enrollments) {
      if (already.get(enrollment.student_id, toSchoolYearId)) { skipped++; continue; }
      insert.run(id('enrollment'), enrollment.student_id, toSchoolYearId, toGradeLevelId, classId, enrollment.id, req.school.id, enrollment.campus_id);
      promoted++;
    }
    db.exec('COMMIT');
...
app.post('/api/admin/promotions/by-class', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  ...
  const fromClass = db.prepare('SELECT id, school_year_id AS schoolYearId FROM classes WHERE id=? AND school_id=?').get(fromClassId, req.school.id);
  const toClass = db.prepare('SELECT id, school_year_id AS schoolYearId, grade_level_id AS gradeLevelId FROM classes WHERE id=? AND school_id=?').get(toClassId, req.school.id);
  if (!fromClass || !toClass) return res.status(400).json({ error: 'Invalid class selection' });
  if (teacherUserId) {
    const validTeacher = db.prepare(`SELECT 1 FROM memberships WHERE user_id=? AND school_id=? AND role='teacher'`).get(teacherUserId, req.school.id);
  ...
  try {
    ...
    if (teacherUserId) {
      ...
      db.prepare(`UPDATE classes SET teacher_user_id=NULL WHERE teacher_user_id=? AND school_id=?`).run(teacherUserId, req.school.id);
      db.prepare(`UPDATE classes SET teacher_user_id=? WHERE id=?`).run(teacherUserId, toClassId);
    }
    const enrollments = db.prepare(`
      SELECT e.id, e.student_id, e.campus_id FROM student_enrollments e
      JOIN students s ON s.id=e.student_id
    ...
    let promoted = 0, skipped = 0;
    for (const enrollment of enrollments) {
      if (already.get(enrollment.student_id, toClass.schoolYearId)) { skipped++; continue; }
      insert.run(id('enrollment'), enrollment.student_id, toClass.schoolYearId, toClass.gradeLevelId, toClassId, enrollment.id, req.school.id, enrollment.campus_id);
      promoted++;
    }
    db.exec('COMMIT');
    res.json({ promoted, skipped });
...
app.get('/api/teacher/parents', requireAuth, requireRole('teacher'), (req, res) => {
  res.json(db.prepare(`
...
app.post('/api/notices', requireAuth, (req, res) => {
  ...
  if (req.user.role === 'teacher') {
    const membership = getMemberships(req.user.id).find(m => m.role === 'teacher');
    ...
    if (targetType === 'PARENT') {
      ...
      const isMyClassParent = db.prepare(`
      ...
      db.prepare(`INSERT INTO notices (id,school_id,campus_id,sender_user_id,sender_name,sender_role,title,body,target_type,target_parent_user_id) VALUES (?,?,?,?,?,?,?,?,?,?)`)
      ...
      db.prepare(`INSERT INTO notices (id,school_id,campus_id,sender_user_id,sender_name,sender_role,title,body,target_type,target_teacher_id) VALUES (?,?,?,?,?,?,?,?,?,?)`)
  ...
  const isSchoolAdmin = getMemberships(req.user.id).some(m => ['school_admin', 'platform_super_admin'].includes(m.role));
  if (!isSchoolAdmin) return res.status(403).json({ error: 'Only a teacher or school admin can send notices.' });
  const membership = getMemberships(req.user.id).find(m => ['school_admin', 'platform_super_admin'].includes(m.role));
  db.prepare(`INSERT INTO notices (id,school_id,campus_id,sender_user_id,sender_name,sender_role,title,body,target_type) VALUES (?,?,?,?,?,?,?,?,?)`)
    .run(id('notice'), membership.schoolId, membership.campusId, req.user.id, req.user.full_name, 'admin', title.trim(), body.trim(), 'SCHOOL');
  res.status(204).end();
});

app.get('/api/me/notices', requireAuth, requireRole('parent'), (req, res) => {
  res.json(db.prepare(`
...
app.post('/api/notices/:id/read', requireAuth, (req, res) => {
  ...
  db.prepare('INSERT OR IGNORE INTO notice_reads (notice_id,user_id) VALUES (?,?)').run(req.params.id, req.user.id);
  res.status(204).end();
});
...
app.get('/api/admin/notices', requireAuth, requireSchoolAccess('school_admin'), (req, res) => {
  res.json(db.prepare(`
    SELECT n.id, n.title, n.body, n.sender_name AS senderName, n.sender_role AS senderRole, n.created_at AS createdAt,
      CASE WHEN nr.user_id IS NULL THEN 0 ELSE 1 END AS read
    FROM notices n
...
app.post('/api/me/guardians', requireAuth, requireRole('parent'), (req, res) => {
  ...
  const myGuardian = db.prepare('SELECT id FROM guardians WHERE user_id=?').get(req.user.id);
  if (!myGuardian) return res.status(403).json({ error: 'No guardian profile found for this account' });
  const membership = getMemberships(req.user.id).find(m => m.role === 'parent');
  if (!membership) return res.status(403).json({ error: 'No active parent membership' });
  const myLinks = db.prepare(`
  ...
  try {
    ...
    if (user) {
      ...
      user = { id: id('parent') }; guardianId = id('guardian');
      db.prepare(`INSERT INTO users (id,full_name,email,phone,password_hash,role) VALUES (?,?,?,?,?,'parent')`)
        .run(user.id, fullName.trim(), email.trim(), phone?.trim() || null, passwordHash(temporaryPassword));
      db.prepare('INSERT INTO guardians (id,user_id) VALUES (?,?)').run(guardianId, user.id);
    }
    db.prepare(`INSERT OR IGNORE INTO memberships (id,user_id,school_id,campus_id,role) VALUES (?,?,?,?, 'parent')`)
    ...
    for (const student of myLinks) link.run(student.studentId, guardianId, relationship.trim(), student.canPickUp, student.canManage);
    ...
    try { db.exec('ROLLBACK'); } catch {}
    res.status(400).json({ error: error.message.includes('UNIQUE') ? 'That email is already registered' : error.message });
  }
});
